...
#[repr(C)]
struct VGAChar {
    _char: u8, // ASCII Characters only
    color: VGAColor,
}
...
impl VGAText {
    ...
    pub fn write_byte(&mut self, _char: u8) {
        match _char {
            ...
            _char => {
                ...
                self.buffer.chars[self.position as usize].write(VGAChar { _char, color });
                self.position += 1;
            }
        }
    }
